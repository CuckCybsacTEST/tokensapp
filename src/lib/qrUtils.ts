...
export class QRUtils {
  ...
  /**
   * Genera un código QR único para un ticket de show
   */
  static async generateTicketQR(ticketData: {
    ...
    // Generar código único
    const qrCode = `TICKET_${randomBytes(8).toString('hex').toUpperCase()}`;

    // Crear datos del QR con información completa del ticket
    const qrData = {
      type: 'show_ticket',
      ticketId: ticketData.id,
      ticketPurchaseId: ticketData.ticketPurchaseId,
      ticketTypeId: ticketData.ticketTypeId,
      showId: ticketData.showId,
      customerDni: ticketData.customerDni,
      customerName: ticketData.customerName,
      customerPhone: ticketData.customerPhone,
      qrCode: qrCode,
      createdAt: new Date().toISOString(),
      // Información del ticket y show para validación
      ticketType: {
        id: ticketData.ticketType.id,
        name: ticketData.ticketType.name,
        price: ticketData.ticketType.price
      },
      show: {
        id: ticketData.ticketType.show.id,
        title: ticketData.ticketType.show.title,
        startsAt: ticketData.ticketType.show.startsAt,
        endsAt: ticketData.ticketType.show.endsAt
      }
    };
  ...
  /**
   * Valida y parsea datos de un código QR de ticket escaneado
   */
  static parseTicketQRData(qrString: string): {
    type: string;
    ticketId: string;
    ticketPurchaseId: string;
    ticketTypeId: string;
    ...
    try {
      const data = JSON.parse(qrString);
      if (data.type === 'show_ticket' && data.ticketId && data.qrCode && data.show && data.ticketType) {
        return data;
      }
      return null;
    } catch {
