...
const MATCHERS: TitleMatcher[] = [
  ...
  { test: p => p.startsWith('/admin/batches/purge'), build: () => 'Purgar Lotes' },
  { test: p => p.startsWith('/admin/batches/'), build: (p) => 'Batch ' + p.split('/').pop() },
  { test: p => p.startsWith('/admin/batches'), build: () => 'Lotes' },
  ...
  { test: p => p.startsWith('/admin/print'), build: () => 'Impresión' },
