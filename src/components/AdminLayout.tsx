...
export function AdminLayout({ children, title, breadcrumbs, basePath = 'admin', hasSession = true }: AdminLayoutProps) {
  ...
  // If at /admin root, render responsive switch purely with CSS (no JS width dependency)
  if (isAdminRoot) {
    return (
      <>
        {/* Mobile-only: card-based panel */}
        <div className="block md:hidden">
          <AdminMobilePanel basePath={basePath} />
        </div>

        {/* Desktop-only: standard sidebar layout */}
        <div className="hidden md:block min-h-screen bg-slate-50 dark:bg-slate-900">
