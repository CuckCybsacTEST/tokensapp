...
async function getMe() {
  ...
  if (!session || session.role !== 'STAFF') return { ok: false as const };
  const user = await prisma.user.findUnique({ where: { id: session.userId }, include: { person: true } });
  if (!user?.person || user.person.area !== 'Caja') return { ok: false as const };
...
export default async function CajaControlPage() {
      ...
      <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-lg border border-slate-100 dark:border-slate-700">
        <div className="flex items-center mb-5 justify-between">
          <div className="flex items-center">
            ...
            <h2 className="text-xl font-bold">Control del Sistema (Caja)</h2>
          </div>
