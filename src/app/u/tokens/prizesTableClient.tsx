...
export default function PrizesTableClient({ onBatchChange }: { onBatchChange?: (batchId:string)=>void }) {
  ...
  <div className="border rounded-lg overflow-hidden w-full">
        <div className="px-4 py-2 text-xs font-semibold bg-slate-100 dark:bg-slate-800 flex items-center justify-between text-slate-900 dark:text-slate-100">
          <span>{label}</span>
        ...
        {list.length === 0 ? (
          ...
          <div className="overflow-x-auto hidden sm:block">
            <table className="w-full text-[11px] table-fixed text-slate-900 dark:text-slate-100">
              ...
              <tbody>
                {list.map(p=>{
                  ...
                  return (
                    <tr key={p.id} className="border-t border-slate-100 dark:border-slate-700/60 hover:bg-slate-50 dark:hover:bg-slate-800/40 text-slate-900 dark:text-slate-100">
          ...
          <div className="sm:hidden divide-y divide-slate-100 dark:divide-slate-700/60 text-[11px] text-slate-900 dark:text-slate-100">
            {list.map(p => (
  ...
  <div className="space-y-3 md:flex md:flex-col md:items-stretch w-full">
        {emitted.length>0 && (
          <div className="flex flex-wrap gap-2">
            <button onClick={()=>{ setActiveBatch('ALL'); onBatchChange?.('ALL'); }} className={`text-[10px] px-3 py-1 rounded border ${activeBatch==='ALL'? 'bg-indigo-600 text-white border-indigo-600':'border-slate-300 dark:border-slate-600 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300'}`}>Todos</button>
            {data.batches.map(b=> (
              <button key={b.batchId} onClick={()=>{ setActiveBatch(b.batchId); onBatchChange?.(b.batchId); }} title={b.batchId} className={`text-[10px] px-3 py-1 rounded border ${activeBatch===b.batchId? 'bg-indigo-600 text-white border-indigo-600':'border-slate-300 dark:border-slate-600 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300'}`}>{(b.description||b.batchId).slice(0,18)}{(b.description||b.batchId).length>18?'…':''}</button>
            ))}
          </div>
        )}
