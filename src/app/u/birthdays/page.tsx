		...
		return (
			<div className="rounded-xl border border-slate-200 dark:border-slate-700 p-4 bg-white dark:bg-slate-800 shadow-sm flex flex-col gap-3">
				...
				<div className="grid gap-y-1 text-[13px] sm:grid-cols-2">
					...
					{/* Fecha de creación oculta en /u/birthdays */}
				</div>
...
export default function StaffBirthdaysPage() {
	...
	async function approve(id:string){ setBusyApprove(p=>({...p,[id]:true})); try { const r=await fetch(`/api/staff/birthdays/${id}/approve`,{method:'POST'}); const j=await r.json(); if(!r.ok) throw new Error(j?.code||j?.message||r.status); load(); } catch(e:any){ setErr(String(e?.message||e)); } finally { setBusyApprove(p=>({...p,[id]:false})); } }
	async function genTokens(id:string){ setBusyGenerate(p=>({...p,[id]:true})); try { const r=await fetch(`/api/staff/birthdays/${id}/tokens`,{method:'POST'}); const j=await r.json(); if(!r.ok) throw new Error(j?.code||j?.message||r.status); load(); } catch(e:any){ setErr(String(e?.message||e)); } finally { setBusyGenerate(p=>({...p,[id]:false})); } }
