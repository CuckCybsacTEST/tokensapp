...
import { Mundial2026FanZoneAdminClient } from "@/app/admin/mundial2026/fanzone/[code]/Mundial2026FanZoneAdminClient";
import { verifyUserSessionCookie } from "@/lib/auth";
import {
  MUNDIAL2026_FANZONE_CAMPAIGN_NAME,
  MUNDIAL2026_FANZONE_MAX_QR_PER_PARTICIPANT,
  formatMundial2026FanZoneExpiresAt,
...
export default async function Mundial2026FanZoneCodePage({ params }: { params: { code: string } }) {
  const session = await verifyUserSessionCookie(cookies().get("user_session")?.value);
  const canValidateDelivery = !!session && ["ADMIN", "COORDINATOR", "STAFF", "COLLAB"].includes(session.role);
    ...
    <div className="min-h-screen bg-[#0E0606] px-3 py-4 text-white sm:px-5 sm:py-6 lg:px-8">
      <div className="mx-auto flex min-h-[calc(100vh-2rem)] w-full max-w-2xl flex-col justify-center gap-4 sm:min-h-[calc(100vh-3rem)] sm:gap-5">
        <div className="flex items-center justify-between gap-3 px-1">
          <Link href="/mundial2026/fanzone" className="text-sm font-medium text-white/65 transition hover:text-white">
            ← Volver
          </Link>
          ...
          <div className="relative rounded-[43px] px-4 py-4 sm:rounded-[49px] sm:px-5 sm:py-5">
            <Mundial2026FanZoneAdminClient
              ticket={{
                ...
                statusLabel,
                expiresAtLabel,
                showValidateButton: canValidateDelivery,
              }}
            />
          </div>
        </section>
