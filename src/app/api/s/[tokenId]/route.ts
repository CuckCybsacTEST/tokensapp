...
export async function GET(_: Request, { params }: { params: { tokenId: string } }) {
  ...
  const rows: any[] = await prisma.$queryRawUnsafe(`
    SELECT
      t.id, t.disabled, t."expiresAt", t."validFrom", t."batchId",
  ...
  const rec = rows[0];
  if (rec.staticTargetUrl == null || rec.staticTargetUrl.trim() === '') return apiError('NOT_FOUND', 'No es un token estático', undefined, 404);
