...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: "BAD_SNAPSHOT" }), { status: 500 });
  }
...
export async function POST(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const result = await prisma.$transaction(async (tx: any) => {
      ...
      if (!session) return { status: 404, body: { error: "NOT_FOUND" } } as const;
      if (session.status === "CANCELLED") {
        return { status: 409, body: { error: "CANCELLED" } } as const;
      }
      if (session.status === "FINISHED") {
        return { status: 409, body: { error: "FINISHED" } } as const;
      }
  let snapshot: SnapshotMeta;
      try {
        snapshot = JSON.parse(session.meta) as SnapshotMeta;
      } catch {
        return { status: 500, body: { error: "BAD_SNAPSHOT" } } as const;
      }
  ...
  await tx.rouletteSpin.create({ data: { sessionId: session.id, prizeId: chosenToken.prizeId, order: nextOrder, weightSnapshot: prizeRemainingBefore, tokenId: chosenToken.tokenId } });
        ...
        return { status: 500, body: { error: 'UNKNOWN_MODE' } } as const;
      }
    });
    ...
    return new Response(JSON.stringify({ error: "SPIN_FAILED" }), { status: 500 });
  }
}
