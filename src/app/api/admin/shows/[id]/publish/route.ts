...
import { publish } from '@/lib/shows/service';
...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  const show = await publish(params.id, { actorRole: (session as any)?.role });
    invalidateShowsCache();
    return NextResponse.json({ ok: true, show: adminFullShape(show) });
  } catch (e: any) {
    const code = e?.code || 'INTERNAL';
    const http = e?.http || (code === 'IMAGE_REQUIRED' ? 400 : code === 'NOT_FOUND' ? 404 : code === 'MAX_PUBLISHED_REACHED' ? 409 : 500);
