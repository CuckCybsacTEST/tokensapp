...
export default function MetricsDashboard({ initialMetrics }: { initialMetrics: any }) {
  ...
  // Forzar período inicial 'today' en primera carga si no coincide
  useEffect(() => {
    if (initialMetrics?.period?.name !== 'today') {
      // Llamada async sin bloquear render inicial
      handlePeriodChange('today').catch(()=>{});
    }
  ...
  return (
    <>
      {/* Métricas por período (ahora primero) */}
          ...
          <div>
              ...
              <div className="flex items-center">
                ...
                <span className="font-medium text-purple-800 dark:text-purple-300">
                  Período: {metrics.period.name === 'custom' 
                    ...
                    : metrics.period.name === 'day_before_yesterday' ? 'Anteayer'
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-4">
              ...
              <StatCard 
                ...
                value={metrics.period.available} 
                description="Sirven dentro del período" 
      ...
      <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-md border border-slate-100 dark:border-slate-700 mb-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-6">
              ...
              <div className="flex-1 bg-white/80 dark:bg-slate-800/50 p-4 rounded-lg shadow-sm">
                ...
                <div className="text-3xl font-bold text-emerald-600 dark:text-emerald-400 flex items-center">
                  ...
                  {metrics.active.toLocaleString()} <span className="text-sm text-slate-500 ml-2">({pct(metrics.active)})</span>
                </div>
              </div>
              <div className="flex-1 bg-white/80 dark:bg-slate-800/50 p-4 rounded-lg shadow-sm">
          ...
          <StatCard label="Tokens Canjeados" value={metrics.redeemed} description={pct(metrics.redeemed) + " del total"} color="text-emerald-600" icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-emerald-500/80" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg>} />
          <StatCard label="Tokens Revelados" value={metrics.revealed} description={pct(metrics.revealed) + " del total"} color="text-violet-600" icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-violet-500/80" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>} />
          <StatCard label="Tokens Expirados" value={metrics.expired} description={pct(metrics.expired) + " del total"} color="text-amber-600" icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-amber-500/80" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>} />
          <StatCard label="Emitidos Históricos" value={metrics.emittedAggregate} description="Total acumulado" color="text-indigo-600" icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-indigo-500/80" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>} />
        </div>
      </div>
    </>
  );
