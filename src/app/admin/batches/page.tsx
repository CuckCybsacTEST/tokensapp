    ...
    <AdminLayout>
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <Link href="/admin/batches/purge" className="btn-outline !px-3 !py-1.5 text-sm" title="Eliminar batches">
            🗑️ Purgar
          </Link>
          <Link href="/admin/print" className="btn-outline !px-3 !py-1.5 text-sm">
            🖨️ Impresión
          </Link>
        </div>
      </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-[11px] text-slate-600 dark:text-slate-400">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex flex-wrap gap-1">
                      ...
                      <Link
                        href={`/admin/print?preselect=${b.id}`}
                        className="btn-outline !px-2 !py-1 text-[10px]"
                        title="Imprimir lote"
                      >
                        🖨️ Imprimir
                    ...
                    {!isStatic && (
                      <div className="flex flex-wrap gap-1 mt-1">
                        {!session && eligibleByPrize && (
                          <Link href={`/admin/batches?createRoulette=1&batch=${b.id}`} className="btn-outline !px-2 !py-1 text-[10px]" title="Crear ruleta por premios">
                            🎡 Ruleta
                          </Link>
                        )}
                        {!session && !eligibleByPrize && eligibleByToken && (
                          <Link href={`/admin/batches?createRoulette=1&batch=${b.id}&mode=token`} className="btn-outline !px-2 !py-1 text-[10px]" title="Crear ruleta por tokens">
                            🎯 Tokens
                          </Link>
                        )}
                        {session && (
