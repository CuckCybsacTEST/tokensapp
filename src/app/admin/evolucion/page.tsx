...
import WeeklyJourneyDashboard from '../daily-evaluation/WeeklyJourneyDashboard';
...
function getHistoricalWeekCount(): number {
  const projectStart = new Date('2025-12-08T12:00:00Z'); // lunes de la primera semana con escaneos
  const days = Math.ceil((Date.now() - projectStart.getTime()) / 86_400_000);
  return Math.ceil(days / 7) + 1;
}

export default function AdminEvolucionPage() {
  const [anchorDay, setAnchorDay] = useState('');
  const [periodIndex, setPeriodIndex] = useState(3); // default: Últimos 2 meses
  const allOptions = [
    ...PERIOD_OPTIONS,
