...
export function BirthdaySearch() {
  ...
  const doSearch = useCallback(
    async (signal?: AbortSignal) => {
      ...
      setErr(null);
      try {
        const res = await fetch(`/api/birthdays/search?q=${encodeURIComponent(val)}`, {
          signal,
          cache: "no-store",
        });
        ...
        if (!res.ok)
          throw new Error((j as any)?.code || (j as any)?.message || `HTTP_${res.status}`);
  ...
  return (
    <div
      className="relative mx-auto z-30 w-[92%] sm:w-[82%] md:w-[70%] lg:w-[58%] max-w-2xl"
      ref={rootRef}
    >
      <div
      ...
      >
        <input
          ...
          aria-activedescendant={
            activeIndex >= 0 && open && items[activeIndex]
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            {loading && <div className="px-3 py-2 text-sm text-white/70">Buscando…</div>}
            {!loading && err && (
            ...
            {!loading && !err && items.length > 0 && (
              <div
                ...
                className="results-list max-h-[55svh] sm:max-h-64 overflow-y-auto custom-scrollbar"
              >
                {items.map((it, idx) => {
                  const href = `/marketing/birthdays/public?rid=${encodeURIComponent(it.id)}`;
                  ...
                  return (
                    ...
                    >
                      <span className="flex-1 flex flex-col">
                        <span className="font-medium group-hover:text-amber-300 transition-colors leading-tight">
                          {it.celebrantName}
                        </span>
                      ...
                      {it.status && (
                        <span className="text-[8.5px] px-1.75 py-[2px] rounded-full bg-white/10 border border-white/15 text-white/70">
                      ...
                      {it.hasCards && (
                        <button
                          ...
                          onClick={(e) => {
                            e.preventDefault();
                            e.stopPropagation();
                            window.location.href = `/marketing/birthdays/${encodeURIComponent(it.id)}/qrs?mode=admin`;
                          }}
      ...
      <style jsx>{`
        .lounge-search {
          ...
          transition:
            box-shadow 0.45s cubic-bezier(0.4, 0.14, 0.2, 1),
            border-color 0.4s,
            background 0.6s;
        }
        ...
        .lounge-search-input:focus-visible {
          outline: none !important;
          box-shadow: none !important;
          border: none !important;
        }
        .lounge-search-input::-moz-focus-inner {
          border: 0;
        }
        .lounge-search-input {
        ...
        .lounge-search.focused .lounge-search-input::placeholder {
          color: rgba(255, 255, 255, 0.24);
        }
        .lounge-search.has-value .lounge-search-input::placeholder {
        ...
        .lounge-chip-clear:hover {
          ...
          color: #fff;
          box-shadow:
            0 4px 14px -4px rgba(255, 100, 40, 0.55),
