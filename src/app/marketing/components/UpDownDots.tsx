      ...
      try {
        ...
        if (lastEl) {
          ...
          const scrolledBottom =
            (window.scrollY || window.pageYOffset || 0) + (window.innerHeight || 0);
  ...
  const Dot = ({ dir, disabled }: { dir: "up" | "down"; disabled?: boolean }) => (
    ...
    >
      ...
      <span aria-hidden className="relative inline-block h-2 w-2 rounded-full bg-white">
        ...
        <span
          className={`absolute left-1/2 -translate-x-1/2 ${dir === "up" ? "-top-2" : "-bottom-2"} h-1 w-1 rounded-full bg-white/80`}
        />
      </span>
    </motion.button>
  );
