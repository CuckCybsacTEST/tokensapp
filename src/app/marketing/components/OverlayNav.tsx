...
export function OverlayNav() {
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      if (y > lastY + 22) setHidden(true); else if (y < lastY - 22) setHidden(false);
      lastY = y;
    };
  ...
  useEffect(() => {
    ...
    const thresholds: number[] = []; for (let t = 0; t <= 1; t += 0.05) thresholds.push(parseFloat(t.toFixed(2)));
  ...
  useEffect(() => {
    ...
    const obs = new IntersectionObserver((entries) => {
      entries.forEach(e => {
        const id = e.target.getAttribute('id'); if (!id) return; ratios.set(id, e.intersectionRatio);
      });
      let bestId = 'hero'; let best = -1;
  ...
  className={`fixed right-4 md:right-5 top-1/2 -translate-y-1/2 z-40 flex flex-col items-center transition-all duration-300 ${inHero ? 'opacity-0 pointer-events-none' : hidden ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
    ...
    >
      ...
      <div
        ...
        style={{ background: 'linear-gradient(to bottom, transparent, rgba(255,255,255,0.20), transparent)' }}
      />
