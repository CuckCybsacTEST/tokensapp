...
type RawPack = { id: string; name: string; qrCount: number; bottle?: string|null; featured?: boolean; perks?: string[]; priceSoles?: number };
type DecoratedPack = Omit<RawPack, 'perks' | 'priceSoles'> & { perks: string[]; priceSoles: number; key: string; accent: string; level: string; highlight: string };
...
export function BirthdaySection() {
  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      } catch(e:any){ if(!cancelled) setError('No se pudieron cargar los Packs'); }
  ...
  useEffect(()=>{
    ...
    if (window.innerWidth >= 640) { setShowHint(false); return; }
    const sync = ()=>{
      const slides = Array.from(el.querySelectorAll('[data-pack-slide]')) as HTMLElement[];
      const center = el.scrollLeft + el.clientWidth/2;
      let best = 0; let dist = Infinity;
      slides.forEach((s,i)=>{const c = s.offsetLeft + s.offsetWidth/2; const d = Math.abs(center-c); if(d<dist){dist=d; best=i;}});
      setActiveIdx(best);
    };
    ...
    return ()=>{ el.removeEventListener('scroll', sync); el.removeEventListener('touchstart', hide); clearTimeout(t); };
  },[]);
                ...
                return (
                  <div key={c.key} data-pack-slide className="w-full flex-shrink-0 snap-center">
                    <motion.div
                      ...
                      style={{ boxShadow: active?`0 14px 42px -18px ${c.accent}90`:`0 8px 26px -16px ${c.accent}55` }}
                    >
                      ...
                      <div className="mt-2 grid grid-cols-3 gap-2 text-[10px] font-medium opacity-80">
                        <div className="flex flex-col"><span className="uppercase opacity-60">Precio</span><span className="text-xs font-bold">S/ {c.priceSoles}</span></div>
                        <div className="flex flex-col"><span className="uppercase opacity-60">Botella</span><span className="text-[11px] font-semibold line-clamp-2">{c.bottle}</span></div>
                        <div className="flex flex-col"><span className="uppercase opacity-60">QRs</span><span className="text-xs font-bold">{c.qrCount}</span></div>
                      </div>
                      <ul className="mt-3 space-y-1.5 text-[12px] opacity-90">
                        {c.perks.slice(0,4).map(p=> (
                          <li key={p} className="flex items-start gap-2"><span className="mt-0.5 text-[9px]" style={{color:c.accent}}>●</span><span className="truncate">{p}</span></li>
                        ))}
                      ...
                      <div className="mt-auto pt-3">
                        <button
                          ...
                          style={{background:`${c.accent}30`,border:'1px solid rgba(255,255,255,0.16)',boxShadow:`0 6px 16px -10px ${c.accent}`}}
                        >Reservar</button>
                      </div>
                    </motion.div>
                  </div>
            ...
            {!loading && decorated.length>1 && (
              <div className="mt-3 packs-dots flex justify-center gap-2">
                {decorated.map((_,i)=>{
                  const active = i===activeIdx;
                  return (
                    <button key={i} aria-label={`Ver pack ${i+1}`}
                      onClick={()=>{
                        const el = sliderRef.current; if(!el) return;
