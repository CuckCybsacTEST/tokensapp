    ...
    (async () => {
      ...
      if (!cs && mode === "admin") {
        ...
        try {
            ...
            if (secRes.ok) {
              ...
              if (newCs) {
                ...
                const pubRes = await fetch(
                  `/api/birthdays/reservations/${id}/tokens?clientSecret=${encodeURIComponent(newCs)}`
                );
          ...
          if (!tRes.ok)
            throw new Error(tJson?.code || tJson?.message || "No se pudieron cargar tokens");
      ...
      if (!cs) {
        setError("Falta el parámetro de seguridad. Vuelve al formulario.");
        setLoading(false);
        return;
      }
      setLoading(true);
      ...
      try {
        const res = await fetch(
          `/api/birthdays/reservations/${id}/tokens?clientSecret=${encodeURIComponent(cs)}`
        );
  ...
  useEffect(() => {
    (async () => {
      ...
      for (const t of tokens) {
        ...
        const url =
          typeof window !== "undefined" ? `${location.origin}/b/${t.code}` : `/b/${t.code}`;
        try {
    ...
    return (
      <section className="container mx-auto max-w-3xl px-4 py-12">
        ...
        <p className="opacity-80 mt-2">
          Estamos preparando tus accesos. Esto puede tardar unos segundos.
        </p>
      </section>
    );
  }
  ...
  if (error) {
    ...
    return (
      <section className="container mx-auto max-w-2xl px-4 py-12">
        ...
        <p className="opacity-80 mt-2">
          {isAuthErr
            ? "Tu enlace de seguridad expiró o es inválido."
            : isRate
              ? "Estás haciendo muchas solicitudes. Intenta nuevamente en un momento."
              : error}
        </p>
        <div className="mt-4 flex gap-3">
          <a
            href="/marketing/birthdays/reservar"
          ...
          <button
            ...
            className="rounded px-4 py-2 font-semibold border border-white/20"
          >
            Reintentar
          </button>
        </div>
  ...
  return (
    <section className="container mx-auto max-w-4xl px-4 py-10">
      <div className="mb-4">
        <a
          href="/"
          className="inline-block rounded px-4 py-2 text-sm font-semibold border border-white/20 hover:bg-white/10 transition-colors"
        >
          Volver al inicio
        </a>
      </div>
            ...
            <div className="snap-center shrink-0 w-full" style={{ scrollSnapAlign: "center" }}>
              ...
              >
                ...
                <div
                  ...
                  style={{
                    background:
                      "linear-gradient(135deg,rgba(212,175,55,0.25),rgba(212,175,55,0.05) 40%,rgba(255,255,255,0) 70%)",
                    boxShadow:
                      "inset 0 0 0 1px rgba(255,236,190,0.08),0 0 0 1px rgba(212,175,55,0.4)",
                  }}
                />
                  ...
                  <div className="relative w-full flex items-center justify-center">
                    ...
                    {cardFailed[host.id] &&
                      (qrData[host.id] ? (
                        ...
                        <div
                          className="w-56 h-56 rounded-xl animate-pulse absolute"
          ...
          {guest && (
            <div className="snap-center shrink-0 w-full" style={{ scrollSnapAlign: "center" }}>
              ...
              >
                    ...
                    {cardFailed[guest.id] &&
                      (qrData[guest.id] ? (
                        <img
                          src={qrData[guest.id]}
                          alt="QR invitados"
                          data-testid="qr-guest-img"
                          className="w-56 h-56 bg-white p-2 rounded-xl absolute"
                        />
                      ) : (
                ...
                <div className="mt-4 flex flex-wrap gap-2 justify-center">
                  <a
                    href={`/api/birthdays/invite/${guest.code}/card?fmt=png`}
                    download
                    className="rounded px-3 py-2 text-sm font-semibold"
                    style={{ background: "#3D2EFF" }}
                  >
                  ...
                  <button
                    onClick={() => copyLink(guest.code)}
                    className="rounded px-3 py-2 text-sm font-semibold border border-white/20"
                  >
                    Copiar link
                  </button>
                </div>
          ...
          <div className="mt-4 flex items-center justify-center gap-2">
            {[host, guest].filter(Boolean).map((_, i) => (
              <button
                ...
                className={`h-2.5 w-2.5 rounded-full transition-all ${activeSlide === i ? "bg-white scale-110" : "bg-white/30 hover:bg-white/60"}`}
              />
            ))}
          </div>
        )}
