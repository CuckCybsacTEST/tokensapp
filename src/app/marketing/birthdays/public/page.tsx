...
export default function PublicBirthdayStatusPage() {
  ...
  function statusBadge(st: string) {
    ...
    switch (st) {
      ...
      case "completed":
        return base + " bg-blue-600/20 border-blue-500/40 text-blue-300";
      case "canceled":
        return base + " bg-rose-600/20 border-rose-500/40 text-rose-300";
      default:
        return base + " bg-slate-600/20 border-slate-500/40 text-slate-300";
    }
  }
  ...
  return (
    <section
      ...
      style={{ background: "linear-gradient(180deg,#0d0d0f,#121218 60%,#0d0d0f)" }}
    >
      <div className="max-w-xl mx-auto space-y-6">
        <h1 className="text-2xl font-extrabold tracking-tight">Estado de tu Reserva</h1>
        {!rid && (
          <p className="text-sm opacity-70">
            Falta el parámetro <code>rid</code>. Accede desde el buscador.
          </p>
        )}
        {rid && !loading && !reservation && !error && (
          <p className="text-sm opacity-70">
            No se encontró la reserva. Verifica el identificador.
          </p>
        )}
        {loading && <p className="text-sm animate-pulse opacity-80">Cargando…</p>}
        {error && (
          <div className="text-sm border border-rose-600 bg-rose-900/30 p-3 rounded">{error}</div>
        )}
        {reservation && (
          <div className="rounded-xl border border-white/10 bg-white/5 backdrop-blur p-4 space-y-3">
            ...
            <div className="text-sm mt-2">
              ...
              {reservation.status === "approved" && (
                <p>
                  ¡Tu reserva fue aprobada! Tus tarjetas QR estarán disponibles pronto (si no las
                  ves, consulta con el staff).
                </p>
              )}
            ...
            <div className="pt-2 border-t border-white/10 text-xs opacity-50">
